...
namespace CyberIDMKing
{
    internal class Core
    {
        public static bool AutoResetTrial {  get; private set; }
        public static DateTime LastReset { private set; get; }
        ...
        private static string RegRead(string keyPath,string valueName)
        {
            using (var src = GetSource(keyPath))
            {
                using (var key=src.OpenSubKey(keyPath.Substring(keyPath.IndexOf("\\") + 1).Trim()))
                {
                    return key.GetValue(valueName)?.ToString()??"false";
                }
            }
        }
        ...
        internal static void SetAutoResetTrial(bool autoResetTrial)
        {
            AutoResetTrial = autoResetTrial;
            RunWait($"reg add \"HKCU\\Software\\DownloadManager\" /v \"auto_trial\" /t \"REG_SZ\" /d \"{autoResetTrial}\" /f");
        }
        internal static void Init()
        {
            bool.TryParse(RegRead("HKEY_CURRENT_USER\\Software\\DownloadManager\\", "auto_trial"), out var autoResetTrial);
            AutoResetTrial = autoResetTrial;
        }
    }
