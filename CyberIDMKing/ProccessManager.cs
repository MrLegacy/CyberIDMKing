...
{
    internal class ProccessManager
    {
        public static Process GetIDMProccess()
        {
            return Process.GetProcessesByName("IDMan").FirstOrDefault();
        }

        public static void RestartIDM()
        {
            try
            {
                var idmProc = GetIDMProccess();
                string idmPath;
                if (idmProc != null)
                {
                    idmPath = idmProc.MainModule.FileName;
                    idmProc.Kill();
                    RunAsDesktopUser(idmPath);
                }
            }
            catch { }
        }
